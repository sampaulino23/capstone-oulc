...
const requestercontroller = {
    ...
    postCreateRequest: async (req, res) => {
        try {
            ...
            var contractrequest = new ContractRequest({
                ...
                isCustomSignatoryLevel: req.body.isCustomSignatoryLevel, 
    ...
    getViolationReportDate: async (req, res) => {
        try {
            ...
            const contractrequests = await ContractRequest.find({requester: req.user._id, requestDate: {$gte: updatedstartdate, $lte: updatedenddate}}).lean()
            .populate({
                path: 'contractType'
            }).exec();
            const contractTypes = await ContractType.find({}).lean().exec();
